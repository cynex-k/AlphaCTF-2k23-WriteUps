...
flag = "AlphaCTF{f1Lter5_b1T_fun}"

flag_bin = "".join((lambda i: bin(ord(i))[2:].zfill(7))(i) for i in flag)
...
def apply_png_filters(rgb_data, width, height):
    ...
    for y in range(height):
        ...
        if y < len(flag_bin): filtered.append(int(flag_bin[y]))  # Always filter type 0 (none!)        
        else: filtered.append(0)
...
if __name__ == "__main__":
    ...
    input_rgb_data = open("hello.rgb", "rb").read()
    ...
    with open("hello1.png", "wb") as f:  # open file for writing
